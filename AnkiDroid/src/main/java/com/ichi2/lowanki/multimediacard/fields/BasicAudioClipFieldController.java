 ...
 ****************************************************************************************/
package com.ichi2.lowanki.multimediacard.fields;
...
public class BasicAudioClipFieldController extends FieldControllerBase implements IFieldController {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if ((resultCode != Activity.RESULT_CANCELED) && (requestCode == ACTIVITY_SELECT_AUDIO_CLIP)) {
            try {
                ...
                LowkeyAnkiDroidApp.sendExceptionReport(e, "handleAudioSelection:unhandled");
                UIUtils.showThemedToast(LowkeyAnkiDroidApp.getInstance().getApplicationContext(),
                        LowkeyAnkiDroidApp.getInstance().getString(R.string.multimedia_editor_something_wrong), true);
            }
        }
    }
    ...
    private void handleAudioSelection(Intent data) {
        ...
        try (Cursor cursor = mActivity.getContentResolver().query(selectedClip, queryColumns, null, null, null)) {
            ...
            if (audioClipFullNameParts.length < 2) {
                try {
                    ...
                    LowkeyAnkiDroidApp.sendExceptionReport(e, "Audio Clip addition failed. Name " + audioClipFullName + " / cursor mime type column type " + cursor.getType(2));
                    UIUtils.showThemedToast(LowkeyAnkiDroidApp.getInstance().getApplicationContext(),
                            LowkeyAnkiDroidApp.getInstance().getString(R.string.multimedia_editor_something_wrong), true);
                    return;
                }
        ...
        try {
            ...
            LowkeyAnkiDroidApp.sendExceptionReport(e, "handleAudioSelection:tempFile");
            UIUtils.showThemedToast(LowkeyAnkiDroidApp.getInstance().getApplicationContext(),
                    LowkeyAnkiDroidApp.getInstance().getString(R.string.multimedia_editor_something_wrong), true);
            return;
        }
        ...
        try (InputStream inputStream = mActivity.getContentResolver().openInputStream(selectedClip)) {
            ...
            LowkeyAnkiDroidApp.sendExceptionReport(e, "handleAudioSelection:copyFromProvider");
            UIUtils.showThemedToast(LowkeyAnkiDroidApp.getInstance().getApplicationContext(),
                    LowkeyAnkiDroidApp.getInstance().getString(R.string.multimedia_editor_something_wrong), true);
        }
    }
