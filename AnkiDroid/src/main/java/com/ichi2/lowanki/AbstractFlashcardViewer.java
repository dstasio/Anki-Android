...
import com.ichi2.lowanki.cardviewer.GestureTapProcessor;
import com.ichi2.lowanki.cardviewer.MissingImageHandler;
import com.ichi2.lowanki.dialogs.TagsDialog;
import com.ichi2.lowanki.multimediacard.AudioView;
import com.ichi2.lowanki.cardviewer.CardAppearance;
import com.ichi2.lowanki.receiver.SdCardReceiver;
import com.ichi2.lowanki.reviewer.CardMarker;
import com.ichi2.lowanki.reviewer.ReviewerCustomFonts;
import com.ichi2.lowanki.reviewer.ReviewerUi;
import com.ichi2.lowanki.cardviewer.TypedAnswer;
...
import static com.ichi2.lowanki.cardviewer.CardAppearance.calculateDynamicFontSize;
import static com.ichi2.lowanki.cardviewer.ViewerCommand.*;
import static com.ichi2.lowanki.reviewer.CardMarker.*;
...
public abstract class AbstractFlashcardViewer extends NavigationDrawerActivity implements ReviewerUi, CommandProcessor {
    ...
    private final Runnable longClickTestRunnable = new Runnable() {
        ...
        public void run() {
            ...
            CompatHelper.getCompat().vibrate(LowkeyAnkiDroidApp.getInstance().getApplicationContext(), 50);
    ...
    private int getRecommendedEase(boolean easy) {
        try {
            ...
            LowkeyAnkiDroidApp.sendExceptionReport(e, "AbstractReviewer-getRecommendedEase");
    ...
    protected void initLayout() {
        ...
        String answerButtonsPosition = LowkeyAnkiDroidApp.getSharedPrefs(this).getString(
    ...
    protected SharedPreferences restorePreferences() {
        ...
        mGesturesEnabled = LowkeyAnkiDroidApp.initiateGestures(preferences);
    ...
    private void recreateWebView() {
        if (mCardWebView == null) {
            ...
            WebViewDebugging.initializeDebugging(LowkeyAnkiDroidApp.getSharedPrefs(this));
    ...
    private void updateCard(final String newContent) {
        ...
        if (LowkeyAnkiDroidApp.getSharedPrefs(this).getBoolean("html_javascript_debugging", false)) {
            try {
     ...
     *
     * @param doAudioReplay indicates an lowanki desktop-like replay call is desired, whose behavior is identical to
            ...
            if (mGesturesEnabled) {
                try {
                    ...
                    if (Math.abs(dx) > Math.abs(dy)) {
                        ...
                        if (dx > LowkeyAnkiDroidApp.sSwipeMinDistance
                                && Math.abs(velocityX) > LowkeyAnkiDroidApp.sSwipeThresholdVelocity
                                && !mIsXScrolling && !mIsSelecting) {
                            // right
                            executeCommand(mGestureSwipeRight);
                        } else if (dx < -LowkeyAnkiDroidApp.sSwipeMinDistance
                                && Math.abs(velocityX) > LowkeyAnkiDroidApp.sSwipeThresholdVelocity
                                && !mIsXScrolling && !mIsSelecting) {
                            // left
                        ...
                        if (dy > LowkeyAnkiDroidApp.sSwipeMinDistance
                                && Math.abs(velocityY) > LowkeyAnkiDroidApp.sSwipeThresholdVelocity
                                && !mIsYScrolling) {
                            // down
                            executeCommand(mGestureSwipeDown);
                        } else if (dy < -LowkeyAnkiDroidApp.sSwipeMinDistance
                                && Math.abs(velocityY) > LowkeyAnkiDroidApp.sSwipeThresholdVelocity
                                && !mIsYScrolling) {
                            // up
    ...
    protected void onFlag(Card card, @FlagDef int flag) {
        ...
        /* Following code would allow to update value of {{cardFlag}}.
           ...
           currently this code would do something that lowanki itself
        ...
        private boolean filterUrl(String url) {
            ...
            switch (signalOrdinal) {
                ...
                case WebViewSignalParserUtils.RELINQUISH_FOCUS:
                    ...
                    return true;
                /*
                 *  Call displayCardAnswer() and answerCard() from lowanki deck template using javascript
