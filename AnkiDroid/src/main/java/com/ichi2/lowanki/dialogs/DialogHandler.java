...
import com.ichi2.lowanki.NotificationChannels;
    ...
    public void handleMessage(Message msg) {
        ...
        if (msg.what == MSG_SHOW_COLLECTION_LOADING_ERROR_DIALOG) {
            ...
            Runnable confirm = () -> {
                ...
                CollectionHelper.getInstance().getCol(LowkeyAnkiDroidApp.getInstance()).modSchemaNoCheck();
            };
            dialog.setConfirm(confirm);
            dialog.setArgs(msgData.getString("message"));
            (mActivity.get()).showDialogFragment(dialog);
        } else if (msg.what == MSG_DO_SYNC) {
            SharedPreferences preferences = LowkeyAnkiDroidApp.getSharedPrefs(mActivity.get());
