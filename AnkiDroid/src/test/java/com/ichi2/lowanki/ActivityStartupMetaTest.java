...
public class ActivityStartupMetaTest extends RobolectricTest {
    ...
    public void ensureAllActivitiesAreTested() throws PackageManager.NameNotFoundException {
        ...
        Object[] manifestActivityNames = Arrays.stream(manifestActivities)
                .map(x -> x.name)
                .filter(x -> !x.equals("com.ichi2.lowanki.TestCardTemplatePreviewer"))
                .filter(x -> !x.equals("com.ichi2.lowanki.AnkiCardContextMenuAction"))
                .filter(x -> !x.equals("com.ichi2.lowanki.analytics.AnkiDroidCrashReportDialog"))
