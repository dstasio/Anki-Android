...
public class ProductionCrashReportingTreeTest {
     ...
     * <p>
     * Info: always {@link LowkeyAnkiDroidApp#TAG} as the logging tag
    ...
    public void testProductionLogTag() {
        ...
        try (MockedStatic<Log> autoClosed = mockStatic(Log.class)) {
            ...
            AnkiAssert.assertDoesNotThrow(() ->
                    autoClosed.verify(() -> Log.i(LowkeyAnkiDroidApp.TAG, "info level message", null)));
            AnkiAssert.assertDoesNotThrow(() ->
                    autoClosed.verify(() -> Log.w(LowkeyAnkiDroidApp.TAG, this.getClass().getSimpleName() + "/ " + "warn level message", null)));
            AnkiAssert.assertDoesNotThrow(() ->
                    autoClosed.verify(() -> Log.e(LowkeyAnkiDroidApp.TAG, this.getClass().getSimpleName() + "/ " + "error level message", null)));
        }
    }
}
