...
import com.ichi2.lowanki.dialogs.RecursivePictureMenu.Item;
import com.ichi2.lowanki.dialogs.RecursivePictureMenu.ItemHeader;
...
public class HelpDialog {
    ...
    private static void openManual(AnkiActivity ankiActivity) {
        ankiActivity.openUrl(Uri.parse(LowkeyAnkiDroidApp.getManualUrl()));
    }

    private static void openFeedback(AnkiActivity ankiActivity) {
        ankiActivity.openUrl(Uri.parse(LowkeyAnkiDroidApp.getFeedbackUrl()));
    }
    ...
    public static DialogFragment createInstance(Context context) {
        ...
        if (!IntentUtil.canOpenIntent(context, LowkeyAnkiDroidApp.getMarketIntent(context))) {
    ...
    public static class RateAppItem extends Item implements Parcelable {
        ...
        public void execute(AnkiActivity activity) {
            IntentUtil.tryOpenIntent(activity, LowkeyAnkiDroidApp.getMarketIntent(activity));
        }
